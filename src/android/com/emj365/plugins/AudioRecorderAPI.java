...
public class AudioRecorderAPI extends CordovaPlugin {
  ...
  public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
    ...
    if (action.equals("stopPlayback")) {
        mp.pause();
        mp.seekTo(0);
        if(!mp.isPlaying()) cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                callbackContext.success("stopPlaybackComplete");
            }
        });
      return true;
    }

    if (action.equals("pausePlayback")) {
        mp.pause();
        if(!mp.isPlaying()) cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                callbackContext.success("pausePlaybackComplete");
            }
        });
      return true;
    }
